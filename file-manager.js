...
function replace(fileName, oldWord, newWord) {
	...
	fs.readFile(fileName, function(err, data) {
		...
		// data.replace(oldWord, newWord);		//only does this for the first oldWord it finds

		data = data.split(oldWord).join(newWord); //splits on the oldWord and joins in the newWord there intstead. works for all instances of oldWord.
